...
#    def process_tree(self, url, auth_data, cur_dir, url_list, test_download=False):
#        #print "Walking %s in %s" % (url, cur_dir)
#        r = requests.get(url, auth=(auth_data['id'], auth_data['pw']), verify=False)
#        if r.status_code == 200:
#            elements = html.from_string(r.content)
...
#                        self.process_tree(url + dir_name, auth_data, cur_dir + dir_name, url_list)
                    # file
#                    else:
#                        if image['alt'].find("DIR") == -1:
...
#                        self.process_tree(url + dir_name, auth_data, cur_dir + dir_name, url_list)
    ...
    def create_tarball_file_node(
        ...
        results = pq.nodes(mod.File).props(name=tarball_name).all()
        if len(results) == 0:
            print "New file, registering with gdcapi"
            # register the files with gdcapi
            #signpost = SignpostClient(os.environ['QA_SIGNPOST_URL'])
            ...
            if len(results) > 1:
                ...
                print "File exists, should use that id, but we're quitting now"
    ...
    def create_related_file_node(self, pq, signpost, entry, project, participant_barcode, tarball_file_node):
        if tarball_file_node != None:
            ...
            if len(results) == 0:
                ...
                assoc_file_url = self.create_signpost_uri(
    ...
    def process_job(self, directory, object_store, bucket, auth_data, project, re_md5_tarball=True):
        ...
        if file_key == None:
            ...
            if re_md5_tarball:
                ...
                # TODO: load these from a data file if possible
                tarball_md5_sum = "bb40c2afdec1a963465dd5a35c1bf887"
                tarball_size = 130294209526
    ...
    def process_all_work(self, ask_for_nih_creds=False, re_md5_tarball=True):
